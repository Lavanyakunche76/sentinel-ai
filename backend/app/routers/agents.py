...
def create_agent(payload: AgentCreate, db: Session = Depends(get_db), _user=Depends(require_role("admin", "security_lead"))):
...
def delete_agent(agent_id: str, db: Session = Depends(get_db), _user=Depends(require_role("admin"))):
    agent = db.query(Agent).filter(Agent.id == agent_id).first()
    if not agent:
        raise HTTPException(404, "Agent not found")
    db.delete(agent)
